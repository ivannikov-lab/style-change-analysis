import re
from ..utils import print_progress_bar, lemmatizer

from nltk.corpus import stopwords
import numpy as np

FUNCTION_WORDS_FILE = 'data/external/function_words/function_arm.txt'
...
def frequent_words_diff(data, window, use_func_words, use_stop_words, local_diff, feature_names=[]):
    ...
    if(use_stop_words):
        frequent_words = list(set(stopwords.words('armenian') + frequent_words))

    vectors = []

    data_length = len(data)
    for i, entry in enumerate(data):
        entry = re.sub("[^\u0561-\u0587\u0531-\u0556]+", " ", entry).lower()

        words = lemmatizer(entry)
